...
function sl_enqueue_scripts() {
	wp_enqueue_script( 'simple-likes-public-js', get_template_directory_uri() . '/js/lib/simple-likes-public.js', array( 'jquery' ), '0.5', false );
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'like' => __( 'Like', 'html5blank' ),
		'unlike' => __( 'Unlike', 'html5blank' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'html5blank' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	// Loader
	$loader = '<span class="sl-loader"><i id="icon-gear" class="fa fa-spin fa-cog"></i></span>';
	// Liked/Unliked Variables
	if ( already_liked( $post_id, $is_comment ) ) {
		$class = esc_attr( ' liked' );
		$title = __( 'Unlike', 'html5blank' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'html5blank' );
		$icon = $icon_empty;
	}
	$output = '<a href="' . admin_url( 'admin-ajax.php?action=process_simple_like' . '&post_id=' . $post_id . '&nonce=' . $nonce . '&is_comment=' . $is_comment . '&disabled=true' ) . '" class="sl-button' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon . $loader . $count . '</a>' ;
	return $output;
...
function get_liked_icon() {
	...
	$icon = '<i id="icon-like" class="fa fa-heart-o"></i>&nbsp;';
	return $icon;
} // get_liked_icon()
...
function get_unliked_icon() {
	...
	$icon = '<i id="icon-unlike" class="fa fa-heart"></i>&nbsp;';
	return $icon;
} // get_unliked_icon()
